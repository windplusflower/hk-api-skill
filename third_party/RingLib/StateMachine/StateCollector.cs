...
namespace RingLib.StateMachine
{
    ...
    public class StateCollector
    {
        ...
        private static void CollectStates()
        {
            ...
            {
                Log.LogInfo(
                    typeof(StateCollector).Name,
                    $"Collecting states for {stateMachineType.Name}"
                );
                ...
                foreach (var method in methods)
                {
                    if (method.GetCustomAttributes(typeof(StateAttribute), false).Length > 0)
                    {
                        ...
                        if (forbiddenNames.Contains(method.Name))
                        {
                            Log.LogError(
                                typeof(StateCollector).Name,
                                $"Forbidden state name {method.Name}"
                            );
                            continue;
                        }
                        Log.LogInfo(
                            typeof(StateCollector).Name,
                            $"    Collected state {method.Name}"
                        );
        ...
        public static Dictionary<string, State> GetStates(StateMachine stateMachine)
        {
            ...
            {
                var stateDelegate = Delegate.CreateDelegate(
                    typeof(State),
                    stateMachine,
                    state.Value
                );
