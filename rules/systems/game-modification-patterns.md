...
| 模式 | 用途 | 链接 |
|------|------|------|
| 商店 FSM 修改 | 修改游戏商店系统 | [#1](#1-商店-fsm-修改模式) |
| Boss 组件附加 | 修改 Boss 行为 | [#2](#2-boss-组件附加模式) |
| Boss 名称修改 | 修改 Boss 显示名称 | [#3](#3-boss-名称修改模式) |
| 敌人预加载 | 预加载和生成敌人 | [#4](#4-敌人预加载模式) |
| 死亡事件订阅 | 监听敌人死亡 | [#5](#5-敌人死亡事件订阅模式) |
| 场景状态管理 | 管理场景状态和分支 | [#6](#6-场景状态管理模式) |
| 文本本地化 | 多语言支持 | [#7](#7-文本本地化模式) |
| 预加载管理 | 批量预加载资源 | [#8](#8-预加载管理模式) |

## 本文档范围

本文档包含**游戏系统特定的修改模式**，针对空洞骑士的具体游戏系统：
- 商店系统修改
- Boss 行为修改
- 敌人生成和管理
- 场景状态管理
...
**不包含**：通用代码模式（如 Hook 注入、对象池、数据访问等）。这些请参考 [Code Patterns](../development/code-patterns.md)。

**FSM 基础操作**：详细的 FSM 操作示例请参考 [FSM Reference](../core/fsm-reference.md#代码示例)。
...
### Fallback Learning (2026-05-30)
<!-- evolution:4e5c4951c565 -->
