...
8. 推荐检索顺序是：先用有碰撞体积的候选对象确认主体，再沿该对象的父子层级查找直接关联物品，最后再结合坐标邻近性和命名模式扩展查找零散分布的相关装饰。只有前两步不能完整解释画面表现时，才把周边独立装饰提升为主要候选。
9. 用碰撞体积锁定主体时，不要只看“有无 collider”。还要继续结合主体名称、层级语义，以及碰撞箱的形状和尺寸判断它是否真的是目标物品本体。名字明显偏向触发器、区域标记、通用容器，或碰撞箱尺寸/长宽比明显更像检测区而不是物品本体时，应降低其优先级，继续核对相邻候选。
